...
router.get('/dashboard', authenticate, requireRole('ADMIN'), async (req, res) => {
  try {
    ...
    const [
      ...
      prisma.order.count({ where: { status: { in: ['DELIVERED', 'COLLECTED'] } } }),
      prisma.order.count({ where: { status: 'CANCELLED' } }),
      prisma.order.aggregate({ where: { createdAt: { gte: todayStart }, status: { not: 'CANCELLED' } }, _sum: { totalPrice: true } }),
      prisma.order.aggregate({ where: { createdAt: { gte: weekStart }, status: { not: 'CANCELLED' } }, _sum: { totalPrice: true } }),
...
router.get('/charts', authenticate, requireRole('ADMIN'), async (req, res) => {
  try {
    ...
    const revenueByDay = await prisma.$queryRaw<{day: string, revenue: number}[]>`
      SELECT 
    ...
    const topDrivers = await prisma.user.findMany({
      ...
      select: {
        id: true, name: true,
        driverOrders: { where: { status: { in: ['DELIVERED', 'COLLECTED'] } }, select: { id: true, totalPrice: true } },
